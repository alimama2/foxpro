...
if(sysinfo) then
	for ele in sysinfo:lines() do
		...
		elseif(valtab[1] == "interface") then
			...
			if(valtab[4]) then intr.count["receive_bytes_origin"] = valtab[4] end
			if(valtab[5]) then intr.count["receive_bytes"] = valtab[5] end
			if(valtab[6]) then intr.count["receive_bytes_speed-origin"] = valtab[6] end
			if(valtab[7]) then intr.count["receive_bytes_speed"] = valtab[7] end
			
			if(valtab[8]) then intr.count["transmit_bytes_origin"] = valtab[8] end
			if(valtab[9]) then intr.count["transmit_bytes"] = valtab[9] end
			if(valtab[10]) then intr.count["transmit_bytes_speed_origin"] = valtab[10] end
			if(valtab[11]) then intr.count["transmit_bytes_speed"] = valtab[11] end
